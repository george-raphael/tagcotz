        ...
        >
          ...
          >
            ...
            >
              ...
              <div>
                <form
                  @submit.prevent="handleKulipia"
                  class="grid grid-cols-1 gap-4 mt-4 pt-4 border-t"
                >
                  <div class="">
                    <InputLabel
                      for="phone_number"
                      class="dark:text-white text-xl text-center w-full"
                    >
                      Weka namba ya simu utakayotumia kufanya
                      malipo:</InputLabel
                    >
                    <TextInput
                      id="phone_number"
                  ...
                  >
                    ...
                    <PrimaryButton :disabled="form.processing" type="submit">
                      <b>
                        {{ form.processing ? "Inachakata..." : "Endelea" }}</b
                      >
                    </PrimaryButton>
                  </div>
                </form>
        ...
        <tbody>
          <tr v-if="res.event">
            ...
            <td class="px-2 py-1 flex items-center gap-1">
              <Link
                v-if="res.event && !res.event.attendance"
                as="button"
                method="POST"
...
function handleKulipia() {
  form.post(
    route("attendance.lipia", { attendance: res.event?.attendance.id }),
    {
      onFinish: () => {
        form.reset();
        closeModal();
