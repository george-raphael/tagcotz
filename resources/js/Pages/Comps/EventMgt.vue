<template>
            ...
            >
              ...
              <div>
                <form
                  ...
                  class="grid grid-cols-1 gap-4 mt-4 pt-4 border-t"
                >
                  <div class="">
                    <InputLabel for="name" class="dark:text-white"
                      >Event Name</InputLabel
                    >
                    <TextInput
                      id="name"
                      v-model="form.name"
                  ...
                  <div class="">
                    <InputLabel for="amount" class="dark:text-white"
                      >Joining Amount</InputLabel
                    >
                    <TextInput
                      id="amount"
                      v-model="form.amount"
                  ...
                  <div class="">
                    <InputLabel for="year" class="dark:text-white"
                      >Year</InputLabel
                    >
                    <TextInput
                      id="year"
                  ...
                  <div class="">
                    <InputLabel for="event_date" class="dark:text-white"
                      >Event Date</InputLabel
                    >
                    <TextInput
                      id="event_date"
                  ...
                  <div>
                    <InputLabel for="status" class="dark:text-white"
                      >Event Status</InputLabel
                    >
                    <select
                      id="status"
                      v-model="form.status"
                  ...
                  <div
                    class="mt-4 mb-3 flex items-center justify-end space-x-5"
                  >
                    <b class="cursor-pointer" @click.prevent="isOpen = false"
                      >Close</b
                    >
  ...
  <div>
    <div class="px-4 py-5 lg:px-12 bg-white">
      ...
      <table class="border border-collapse mt-6 text-gray-500 min-w-full">
        <thead>
          <tr class="">
            ...
            <th
              class="hidden lg:table-cell text-left bg-slate-300 border border-slate-50 py-3 px-2"
            >
              Joining Amount
            </th>
            <th
        ...
        <tbody>
          <tr v-for="(event, index) in res.events" :key="event.id">
            ...
            <td class="hidden lg:table-cell border px-2 border-slate-300">
              <p class="font-bold">{{ currencyFormat(event.amount) }} TZS</p>
            </td>
            <td class="hidden lg:table-cell border px-2 border-slate-300">
              {{ event.year }}
            </td>
            ...
            <td
              class="border px-2 border-slate-300 py-1 flex items-center gap-1 flex-col lg:flex-row"
            >
              <Link
                :href="route('event.view', { event: event.id })"
              ...
              <div class="flex items-center gap-0.5">
                <button
                  ...
                  class="underline text-green-700 m-1"
                >
                  Edit
                </button>
                <button
                  @click.prevent="destroyEvent(event)"
                  class="underline text-red-700 m-1"
                >
                  Delete
                </button>
              </div>
          ...
          <tr v-if="!res.events.length">
            <td colspan="7">
              <p class="text-xl text-center text-slate-500 py-8">
                No events available
              </p>
            </td>
          </tr>
...
function saveEvent() {
  form.post(
    form.id ? route("event.update", { event: form.id }) : route("event.store"),
    {
      onFinish: () => {
        form.reset();
        closeModal();
