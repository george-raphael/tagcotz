                    ...
                    <div class="flex min-h-full items-center justify-center p-4 text-center">
                        <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                            ...
                            <DialogPanel
                                ...
                                <div class="my-12 flex items-center justify-center text-2xl underline">
                                    
                                </div>

                                <div class="mt-4 flex items-center justify-center space-x-2">
                                    <button @click="deleteAttendance"
        ...
        <div class="py-6">
            <div class="max-w-7xl mx-auto sm:px-2 lg:px-4">
                <div>
                    ...
                    /
                    <a :href="route('event.view',{event: res.eventProp.id})" class="text-link font-bold">{{res.eventProp.name}}</a>
                    /
                    ...
                    <div>
                        <div class="px-4 sm:px-12 bg-white">
                            <div class="flex sm:flex-row flex-col justify-between">
                                ...
                                <div class="flex sm:flex-row flex-col-reverse space-x-2 items-center">
                                    <div>
                                        <input class="mt-2" v-model="searchQuery" @keyup="searchAttendees" type="text"
                                            placeholder="Search..." />
                                    </div>
                                    <div class="flex flex-row space-x-2 mt-2 items-center">
                                        <a :href="route('dashboard.print.ids',{event: res.eventProp.id})" target="_blank"
                                            class="px-4 py-2 bg-blue-800 border border-transparent font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-900 focus:outline-none focus:border-blue-900 focus:ring focus:ring-blue-300 disabled:opacity-25 transition flex items-center justify-center rounded-none"><svg
                                        ...
                                        <a :href="route('dashboard.export', { searchQuery: searchQuery, status: status })"
                                            target="_blank"
                                            ...
                                            <svg class="w-5 h-5 text-gray-100" fill="currentColor"
                            ...
                            <div v-if="!isLoading">
                                <table class="border border-collapse mt-6 text-gray-500 min-w-full">
                                    <thead>
                                        <tr>
                                            <th class="text-left bg-slate-300 border border-slate-300 px-2 py-3">
                                                SN.
                                            </th>
                                            <th class="text-left bg-slate-300 border border-slate-300 px-2 py-3">
                                                Name
                                    ...
                                    <tbody>
                                        <tr v-for="(
                                                attendee, index
                                            ) in res.attendeesData.data" :key="attendee.id">
                                            ...
                                            <td class="border px-2 border-slate-300">
                                                <p>
                                                    <a class="text-blue-500 block" :href="'tel: ' +
                                            ...
                                            <td class="border px-2 border-slate-300 text-center">
                                                <span v-if="attendee.status ==
                                                    'unverified'
                                                    "
                                                    class="px-1 block py-1 rounded-full text-xs m-2 bg-red-100 text-red-900">
                                                    {{ attendee.status }}
                                                </span>
                                                <span v-else-if="attendee.status ==
                                                    'verified'
                                                    "
                            ...
                            <div v-else class="w-full">
                                <img class="mx-auto border w-12 bg-blue-400 rounded-full" src="/img/loader.svg" alt="" />
                            </div>
                        </div>
                    </div>
                </div>
